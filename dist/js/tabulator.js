...
(function (global, factory) {
				...
				value: function updateData(updatedData) {
					...
					return new Promise(function (resolve, reject) {
						...
						for (var _attrname in updatedData) {
							var columns =
								_this2.table.columnManager.getColumnsByFieldRoot(_attrname);
							columns.forEach(function (column) {
			...
			},
			{
				...
				value: function displayRows(adjust) {
					...
					var index = this.table.rowManager.displayRows.length - 1,
						lookupIndex;
					if (this._handler) {
						lookupIndex = this.table.rowManager.displayPipeline.findIndex(
							function (item) {
	...
	function fitColumns(columns, forced) {
		var totalWidth =
			this.table.rowManager.element.getBoundingClientRect().width; //table element width
						...
						this.table.element.addEventListener("copy", function (e) {
							...
							if (!_this2.blocked) {
								...
								if (_this2.customSelection) {
									...
									list = _this2.table.modules["export"].generateExportList(
	...
	function html(list, options, setFileContents) {
		if (this.modExists("export", true)) {
			setFileContents(
				this.modules["export"].generateHTMLTable(list),
				"text/html"
			);
		}
	}
			...
			},
			{
				...
				value: function _onRendered() {
					...
					this.input.addEventListener(
						"mousedown",
						this._preventPopupBlur.bind(this)
					);
				},
			},
			{
				...
				value: function _scrollToValue(_char) {
					...
					var match = this.displayItems.find(function (item) {
						return (
							...
							item.label.toLowerCase().startsWith(_this4.filterTerm)
						);
					});
					if (match) {
			...
			},
			{
				...
				value: function _showList() {
					...
					if (this.input.parentNode) {
						...
						if (!startVis) {
							setTimeout(function () {
								_this13.popup.hideOnBlur(
									_this13._resolveValue.bind(_this13, true)
								);
							}, 10);
						}
					}
		...
		_createClass(Export, [
			...
			},
			{
				...
				value: function columnVisCheck(column) {
					...
					return (
						visProp !== false &&
						(column.visible || (!column.visible && visProp))
					);
				},
			},
			{
						...
						switch (row.type) {
							case "header":
								headerEl.appendChild(
									_this6.generateHeaderElement(row, setup, styles)
								);
								break;
							case "group":
								bodyEl.appendChild(
									_this6.generateGroupElement(row, setup, styles)
								);
								break;
							case "calc":
								bodyEl.appendChild(
									_this6.generateCalcElement(row, setup, styles)
								);
								break;
							case "row":
						...
						if (col) {
							...
							cellStyle =
								styles.styleCells && styles.styleCells[index]
									...
									: styles.firstCell;
							if (cellStyle) {
								_this8.mapElementStyles(cellStyle, cellEl, [
									"padding-top",
									"padding-left",
									"padding-right",
									"padding-bottom",
			...
			},
			{
				...
				value: function initializeRow(row) {
					...
					if (rowType === "number") {
						...
						if (
							frozenRows.includes(row.data[this.options("frozenRowsField")])
						) {
							this.freezeRow(row);
						}
					}
				...
				value: function mergeDefinition(oldCols, newCols) {
					...
					newCols.forEach(function (column, to) {
						...
						if (from) {
							...
							if (from.columns) {
								from.columns = _this3.mergeDefinition(
									from.columns,
									column.columns
								);
							}
				...
				value: function parseColumns(columns) {
					...
					columns.forEach(function (column) {
						...
						if (column.isGroup) {
							...
							if (
								_this4.config.columns === true ||
								_this4.config.columns == undefined
							) {
				...
				value: function initialize() {
					if (this.table.options.reactiveData) {
						...
						this.subscribe(
							...
							this.block.bind(this, "rowsave")
						);
						this.subscribe(
							"row-data-save-after",
							this.unblock.bind(this, "rowsave")
						);
		...
		_createClass(ResizeTable, [
			...
			},
			{
				...
				value: function initializeVisibilityObserver() {
					var _this3 = this;
					this.visibilityObserver = new IntersectionObserver(function (
						entries
					) {
